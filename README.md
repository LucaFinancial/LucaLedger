...
cd LucaLedger
pnpm install   # ~40 seconds; do not interrupt
```
...
| -------------- | --------------------------------------------- | ------------------------------------------- |
| `pnpm dev`     | Start Vite dev server (http://localhost:5173) | Hot reload + fast refresh                   |
| `pnpm build`   | Production build to `dist/`                   | ~15 s; copies CNAME + 404 on GH Pages build |
| `pnpm preview` | Serve built assets locally                    | Useful for final QA                         |
| `pnpm lint`    | ESLint with React/A11y rules                  | Required before pushing                     |
| `pnpm test`    | Vitest suite (headless)                       | Use `pnpm test:watch` while developing      |
...
- Use `pnpm test:coverage` for v8 instrumentation reports
...
- `pnpm build:pages` handles the static-site artifacts (`CNAME`, `404.html`)
- `pnpm deploy` uses `gh-pages` to publish the `dist/` directory manually if needed
...
- **Lint errors?** Run `pnpm lint` for actionable rule hints; most issues are autofixable.
